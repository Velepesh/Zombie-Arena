...
public class InfiniteWavesSpawner : ObjectPool
{
    ...
    {
        _minCircleWaveIndex = Math.Clamp(_minCircleWaveIndex, 0, _level.WavesCount - 1);
        _maxWaveIndex = Math.Clamp(_maxWaveIndex, _minCircleWaveIndex, _level.WavesCount - 1);
    }
    ...
    {
        _currentWave = _level.GetWave(index);
    ...
    private void TrySpawnNextWave()
    {
        ...
        {
            if (_currentWaveNumber + 1 >= _level.WavesCount)
                Ended?.Invoke();
            else
                NextWave();
        }
