...
public class ZombiesSpawner : ObjectPool, IZombieSpawner
{
    ...
    public void Init(IReadOnlyList<ISpawnPoint> spawnPoints, Level level, ZombieTargetsCompositeRoot targets, bool isMobilePlatform, IWaveSetter waveSetter)
    {
    ...
    private void Spawn(GameObject zombieObject)
    {
        ...
        if (zombieObject.TryGetComponent(out Zombie zombie))
        {
            ...
            zombie.Init(_targets, _isMobilePlatform);
            zombie.Died += OnDied;
            zombie.Disabled += OnDisabled;
            zombie.HitTaken += OnHitTaken;
            _zombies.Add(zombie);
